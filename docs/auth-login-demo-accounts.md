...
The current login is still frontend mock auth. It is now role-complete for the first release demo and supports the four identities needed to test the platform:
...
| `enterprise` | `123456` | Regular enterprise user | `/workspace` | Normal workspace, credits, and recharge pages. No back-office access. |
...
Back-office access is protected by `menu:admin`. The regular `enterprise` user does not have this permission and will be redirected away from `/credits-admin`.
...
- `enterprise` does not have account creation permission
