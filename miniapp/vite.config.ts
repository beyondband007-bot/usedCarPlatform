...
function reorderTabBarPages() {
  ...
  const homePages = pages.filter((page: any) => page.type === 'home')
  const tabPages = TAB_BAR_PAGE_PATHS
    .map(pagePath => pages.find((page: any) => page.path === pagePath))
    .filter(Boolean)
  const rest = pages.filter((page: any) => page.type !== 'home' && !TAB_BAR_PAGE_PATHS.includes(page.path))
  const nextPages = [...homePages, ...tabPages, ...rest]

  if (JSON.stringify(pages) === JSON.stringify(nextPages))
    return

  config.pages = nextPages
  fs.writeFileSync(pagesJsonPath, stringify(config, null, 2))
}

function reorderTabBarPagesPlugin(): Plugin {
  let reorderTimer: ReturnType<typeof setTimeout> | undefined
  const scheduleReorder = () => {
    clearTimeout(reorderTimer)
    reorderTimer = setTimeout(reorderTabBarPages, 80)
  }

  return {
    name: 'reorder-tab-bar-pages',
    enforce: 'post',
    buildStart: scheduleReorder,
