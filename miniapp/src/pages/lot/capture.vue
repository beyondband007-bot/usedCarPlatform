...
const lotId = ref('')
const lotName = ref('')
const positions = ref<CapturePosition[]>([...DEFAULT_LOT_CAPTURE_POSITIONS])
...
const lotTasks = computed(() => uploadStore.queue.filter(task =>
  task.vehicleId === lotId.value && task.ownerType === 'lot' && task.status !== 'cancelled',
))
const hasUploadingTask = computed(() => lotTasks.value.some(task => ['waiting', 'uploading'].includes(task.status)))
...
function taskFor(code: string): UploadTask | undefined {
  return lotTasks.value.find(task => task.captureCode === code)
}
...
async function loadPositions() {
  ...
  errorMessage.value = ''
  try {
    const lot = await hydrateLotMaterialsFromServer(lotId.value)
    lotName.value = lot.name
  }
  catch {
    errorMessage.value = '车场素材加载失败，请下拉刷新重试'
  }
  finally {
    loading.value = false
...
function remove(position: CapturePosition) {
  ...
  uni.showModal({
    ...
    success: (result) => {
      ...
      void (async () => {
        try {
          await removeLotMaterial(lotId.value, position.code, task)
        }
        catch (error) {
          uni.showToast({
...
onLoad((options) => {
  lotId.value = String(options?.lotId || options?.id || '')
  if (!lotId.value) {
    loading.value = false
    errorMessage.value = '缺少车场参数'
    return
  }
  loadPositions()
  processQueue()
...
<template>
  <view class="min-h-screen bg-[#F5F7FA] pb-28">
    <view class="bg-white px-4 pb-4 pt-4">
      <view v-if="lotName" class="mb-2 text-3.5 text-[#6B7280]">
        {{ lotName }}
      </view>
    ...
    <view class="grid grid-cols-2 gap-3 p-3">
      <view v-for="position in positions" :key="position.code" class="overflow-hidden rounded-3 bg-white">
        <view class="relative h-32 bg-[#FEF3C7]" @click="taskFor(position.code) ? undefined : choose(position)">
          <image
          ...
          <video
            ...
            object-fit="cover"
          />
          <view v-else class="h-full flex flex-col items-center justify-center text-[#D97706]">
            <view class="text-7">
              +
            </view>
            <view class="text-3">
        ...
        <view class="p-3">
          ...
          <view v-if="taskFor(position.code)" class="mt-3 flex gap-2">
            <button class="m-0 h-8 flex-1 rounded-1.5 bg-[#FEF3C7] px-2 text-3 text-[#D97706]" @click.stop="choose(position)">
              重传
            </button>
    ...
    <view class="safe-area-inset-bottom fixed bottom-0 left-0 right-0 border-t border-[#E5E7EB] bg-white p-3">
      <button
        ...
        :class="canSubmit && !hasUploadingTask ? 'bg-[#111827]' : 'bg-[#9CA3AF]'"
        :disabled="finishing"
        @click="submit"
      >
        {{ finishing ? '提交中…' : hasUploadingTask ? '上传中…' : '完成补充' }}
      </button>
    </view>
  </view>
</template>
