...
const wallLogoInstruction =
  "第三张图是 Logo / 标识参考图。背景墙 Logo 只参考第三张图中的文字轮廓、字形结构和图形符号，不参考也不要复制它的底色、底板、材质、外框、边框、螺丝、铆钉、牌匾造型、车牌比例或装饰风格。请将提取出的标识重新设计为展厅后方背景墙上的低调品牌墙标识，例如浅灰金属立体字、白色亚克力字、磨砂银墙面字或轻微背光发光字；标识应直接附着在墙面上，与白色展厅、柔和灯光和简洁高级的装修风格一致。不要生成任何矩形牌子、黑色底牌、金色边框、螺丝固定件或悬挂牌匾，不要让墙面 Logo 像车牌。Logo 尺寸要克制，位置要自然，不要遮挡车辆主体，不要贴到车牌、车身、车窗、地面或前景物体上，不要新增多余文字。";
