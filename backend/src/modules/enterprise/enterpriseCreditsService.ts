...
import { loadFallbackCreditsAccount, pickCreditsAccount } from "../billing/creditsAccountLookupService";
...
export async function getEnterpriseCreditsOverview(
  ...
  const account =
    pickCreditsAccount(accountsResult.accounts, identity) ?? (await loadFallbackCreditsAccount(identity));
