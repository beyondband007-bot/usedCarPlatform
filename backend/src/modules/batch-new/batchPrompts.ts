...
const wallLogoInstruction = (logoImageLabel: string) =>
  `${logoImageLabel}是 Logo / 标识参考图。请将 ${logoImageLabel}中的 Logo / 标识自然放置在目标场景后方背景墙、品牌墙或可用墙面区域，匹配墙面透视、尺寸、材质、光照、阴影和轻微反射；Logo 应像真实展厅品牌标识一样贴合墙面，不要遮挡车辆主体，不要贴到车牌、车身、车窗、地面或前景物体上，不要新增多余文字。`;
